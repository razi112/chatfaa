...
ALTER TABLE public.profiles
  ...
  ADD COLUMN IF NOT EXISTS gender        TEXT CHECK (gender IN ('male','female','non_binary','prefer_not_to_say','custom')),
  ADD COLUMN IF NOT EXISTS gender_custom TEXT,
  ADD COLUMN IF NOT EXISTS account_type  TEXT NOT NULL DEFAULT 'personal' CHECK (account_type IN ('personal','creator','business')),
  ...
  ADD COLUMN IF NOT EXISTS content_prefs JSONB NOT NULL DEFAULT '{
    "dark_mode": true,
    "language": "en",
    "data_saver": false
  }'::jsonb,
  ADD COLUMN IF NOT EXISTS story_privacy  TEXT NOT NULL DEFAULT 'everyone' CHECK (story_privacy IN ('everyone','followers','close_friends','no_one')),
  ADD COLUMN IF NOT EXISTS reel_privacy   TEXT NOT NULL DEFAULT 'everyone' CHECK (reel_privacy IN ('everyone','followers','no_one')),
  ADD COLUMN IF NOT EXISTS tag_permissions TEXT NOT NULL DEFAULT 'everyone' CHECK (tag_permissions IN ('everyone','followers','no_one')),
...
-- ──────────────────────────────────────────────────────────────
-- RPC: update content/appearance preferences
-- ──────────────────────────────────────────────────────────────
...
-- ──────────────────────────────────────────────────────────────
-- RPC: get blocked users list with profile info
-- ──────────────────────────────────────────────────────────────
...
-- ──────────────────────────────────────────────────────────────
-- RPC: get restricted users list with profile info
-- ──────────────────────────────────────────────────────────────
...
-- ──────────────────────────────────────────────────────────────
-- RPC: get muted users list with profile info
-- ──────────────────────────────────────────────────────────────
...
-- ──────────────────────────────────────────────────────────────
-- RPC: restrict / unrestrict user
-- ──────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.restrict_user(_target uuid)
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  IF auth.uid() IS NULL THEN RAISE EXCEPTION 'Not authenticated'; END IF;
  INSERT INTO public.restricted_users(restrictor_id, restricted_id) VALUES(auth.uid(), _target) ON CONFLICT DO NOTHING;
END; $$;
...
-- ──────────────────────────────────────────────────────────────
-- RPC: mute / unmute user
-- ──────────────────────────────────────────────────────────────
CREATE OR REPLACE FUNCTION public.mute_user(_target uuid)
RETURNS void LANGUAGE plpgsql SECURITY DEFINER SET search_path = public AS $$
BEGIN
  IF auth.uid() IS NULL THEN RAISE EXCEPTION 'Not authenticated'; END IF;
  INSERT INTO public.muted_users(muter_id, muted_id) VALUES(auth.uid(), _target) ON CONFLICT DO NOTHING;
END; $$;
...
-- ──────────────────────────────────────────────────────────────
-- RPC: deactivate account (sets a flag, reversible on login)
-- ──────────────────────────────────────────────────────────────
ALTER TABLE public.profiles
